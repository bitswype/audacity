...
unsigned ComputeRecordingDialogColumnCount(
    unsigned deviceChannels, const std::vector<PlaybackInputMask>& trackMasks, const std::vector<unsigned>& trackChannelCounts);

//! Count tracks whose input mask has any bit set at index >=
//! @p deviceChannels.
MIXER_API
unsigned CountRecordingTracksWithBitsAboveDeviceWidth(
    unsigned deviceChannels, const std::vector<PlaybackInputMask>& trackMasks);
