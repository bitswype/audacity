...
DownmixStage::DownmixStage(std::vector<std::unique_ptr<DownmixSource> > downmixSources,
    ...
    // Intermediate buffer needs enough channels for the widest source
    // we'll process.  +1 for the GVerb workaround (Issues 3565, 3854)
    // that needs one extra buffer that is simply discarded.  Minimum 3
    // for backward compatibility with stereo (the old hardcoded value).
