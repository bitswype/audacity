...
MIXER_API void RouteTrackSamples(
    const TrackChannelAssignment& assignment, size_t numSourceChannels, size_t numOutputChannels, size_t samplesAvailable,
    const std::function<float(int)>& getChannelVolume, float* const* processingBuffers, float* const* masterBuffers);
