...
TEST_CASE(
   "WaveTrack XML: legacy outputmask with silent-sentinel (bit 63) -> empty mask",
   "[WaveTrack][OutputMask][XML]")
{
   // The previous schema used bit 63 of a single-word outputmask as
   // the silent sentinel.  Loading such a project should produce an
   // empty mask under the new model, with the seen flag set so the
   // migration doesn't overwrite the user's explicit silent choice.
   const auto track = gTrackMaker.Track(WaveClipHolders {});
   const unsigned long long kLegacySilent = 1ull << 63;
   AttributesList attrs {
      { std::string_view { "outputmask" },
        XMLAttributeValueView { kLegacySilent } }
   };
   REQUIRE(track->HandleXMLTag(WaveTrack::WaveTrack_tag, attrs));
   REQUIRE(track->GetPlaybackOutputMask().empty());
   REQUIRE(track->GetOutputMaskAttrSeen());
...
TEST_CASE(
   "WaveTrack XML: legacy outputmask with normal bits -> lo word",
   "[WaveTrack][OutputMask][XML]")
{
   const auto track = gTrackMaker.Track(WaveClipHolders {});
   const unsigned long long kLegacyValue = 0xC0FFEEull;
   AttributesList attrs {
      { std::string_view { "outputmask" },
        XMLAttributeValueView { kLegacyValue } }
   };
   REQUIRE(track->HandleXMLTag(WaveTrack::WaveTrack_tag, attrs));
   REQUIRE(track->GetPlaybackOutputMask().lo ==
      static_cast<uint64_t>(kLegacyValue));
   REQUIRE(track->GetPlaybackOutputMask().hi == 0);
   REQUIRE(track->GetOutputMaskAttrSeen());
}
