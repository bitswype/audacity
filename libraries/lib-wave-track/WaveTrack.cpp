...
bool WaveTrack::MixDownToMono(
   const std::function<void(double)>& progress,
   const std::function<bool()>& cancel)
{
   WaveClipHolders multiChannelClips;
   std::copy_if(mClips.begin(), mClips.end(),
      std::back_inserter(multiChannelClips),
...
auto WaveTrack::SplitChannels() -> std::vector<Holder>
{
   ...
   // Split off channels N-1..1 into new mono tracks.
   // SplitChannels() peels the last channel each time.
   ...
   // Now this track has only channel 0 remaining.
   // Fix up channel attachments to avoid waste of space.
   for (size_t i = n - 1; i >= 1; --i)
      result[0]->EraseChannelAttachments(i);
   for (size_t ch = 1; ch < n; ++ch) {
      for (size_t i = n - 1; i > ch; --i)
         result[ch]->EraseChannelAttachments(i);
      for (size_t i = 0; i < ch; ++i)
         result[ch]->EraseChannelAttachments(0);
   }
   ...
   if (tag == WaveTrack_tag) {
      ...
      for (const auto& pair : attrs)
      {
         ...
         {}
            ...
            // bitswype fork: low 64 bits of 128-bit recording-input
            // mask.  Same %llu-vs-%lld backstop as OutputMaskLo.
            uint64_t bits = 0;
            if (unsigned long long v = 0; value.TryGet(v))
               bits = static_cast<uint64_t>(v);
            else if (long long sv = 0; value.TryGet(sv))
