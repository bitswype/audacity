...
*/
#include "recordingroutingmodel.h"
...
RecordingRoutingModel::RecordingRoutingModel(QObject* parent)
    : QObject(parent), muse::Contextable(muse::iocCtxForQmlObject(this))
{
}

void RecordingRoutingModel::load()
{
    rebuildFromProject();
...
int RecordingRoutingModel::trackCount() const
{
    return static_cast<int>(m_rows.size());
}

int RecordingRoutingModel::channelCount() const
{
    return m_channelCount;
}

int RecordingRoutingModel::displayChannelCount() const
{
...
bool RecordingRoutingModel::isDeviceChannel(int channel) const
{
    return channel >= 0 && channel < m_channelCount;
}

QString RecordingRoutingModel::trackName(int trackIndex) const
{
...
bool RecordingRoutingModel::isRouted(int trackIndex, int channel) const
{
    if (trackIndex < 0 || trackIndex >= static_cast<int>(m_rows.size())) {
        return false;
    }
...
void RecordingRoutingModel::setRouted(int trackIndex, int channel, bool on)
{
    if (trackIndex < 0 || trackIndex >= static_cast<int>(m_rows.size())) {
        return;
    }
...
void RecordingRoutingModel::resetIdentity()
{
...
void RecordingRoutingModel::clearAll()
{
    for (auto& row : m_rows) {
        row.lo = 0;
        row.hi = 0;
...
void RecordingRoutingModel::apply()
{
    ...
    for (const auto& row : m_rows) {
        ...
        if (waveTrack) {
            ...
            mask.lo = row.lo;
            mask.hi = row.hi;
            waveTrack->SetPlaybackInputMask(mask);
        }
    }
}

void RecordingRoutingModel::rebuildFromProject()
{
    m_rows.clear();
    m_channelCount = std::max(1, AudioIORecordChannels.ReadWithDefault(2));
    ...
    for (const auto& track : trackedit->trackList()) {
        ...
        const auto mask = waveTrack->GetPlaybackInputMask();
        row.lo = mask.lo;
        row.hi = mask.hi;
